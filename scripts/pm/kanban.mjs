...
switch (verb) {
  ...
  case 'status': { const t = find(id); const s = rest[0]; if (!board.columns.includes(s)) throw new Error(`bad status ${s}`); t.status = s; t.column = s; t.updated = now; appendMd(id, 'STATUS', `${s}${rest[1] ? ' — ' + rest.slice(1).join(' ') : ''}`); save(); console.log(`${id} → ${s}`); break; }
  ...
  case 'complete': { const t = find(id); const result = rest.join(' '); t.status = 'done'; t.column = 'done'; t.result = result; t.updated = now; appendMd(id, 'COMPLETE', result); save(); console.log(`${id} → done`); break; }
  case 'block': { const t = find(id); const kind = rest[0]; const reason = rest.slice(1).join(' '); t.status = 'blocked'; t.column = 'blocked'; t.blocker = { kind, reason, at: now }; t.updated = now; appendMd(id, 'BLOCKED', `${kind}: ${reason}`); save(); console.log(`${id} → blocked (${kind})`); break; }
  case 'unblock': { const t = find(id); t.status = rest[0] || 'ready'; t.column = t.status; t.blocker = null; t.updated = now; appendMd(id, 'UNBLOCKED', rest.slice(1).join(' ')); save(); console.log(`${id} → ${t.status}`); break; }
  case 'create': { // kanban_create: JSON on stdin {id?, slug, title, assignee, priority, parents, summary, order[], gates[], verify[], skills?, status?, migrated?, blocked_by?}
    ...
    board.tasks.push({ id: tid, title: spec.title, status, column: status, assignee: spec.assignee, priority: spec.priority, parents: spec.parents, skills: spec.skills || [], hierarchy: { role: H[0], level: H[1], effort: H[2] }, provider: or ? 'openrouter→anthropic-oauth' : 'anthropic-oauth', created: now, updated: now, summary: spec.summary, decisions: [], comments: [], blocker: spec.blocked_by ? { kind: 'hard_fail', reason: spec.blocked_by, at: now } : null });
    save(); console.log(tid); break; }
  case 'ready-check': { // promote todo → ready when every parent is done
    let n = 0; for (const t of board.tasks) if (t.status === 'todo' && t.parents.every((p) => (board.tasks.find((x) => x.id === p) || {}).status === 'done')) { t.status = 'ready'; t.column = 'ready'; t.updated = now; n++; console.log(`ready: ${t.id} ${t.title.slice(0, 70)}`); } save(); console.log(`${n} promoted`); break; }
