...
REALTIME_DONE_MS="$(grep -E 'First token to done latency:' reports/phase21/phase21-realtime-pipeline-report.md | awk -F': ' '{print $2}' | awk '{print $1}')"
...
awk "BEGIN {exit !(${REALTIME_DONE_MS} < 5000)}"
...
- Realtime first-token-to-done: ${REALTIME_DONE_MS} ms
