...
## COMPLETE (2026-09-06T06:08:22.628Z)
PARTIAL (goal_complete:false, honest). Fix 1 LANDED: e92808f on worktree-w3-p0a — the unscoped '.minivic-dock .minivic-launcher{pointer-events:auto}' let the unpainted launcher own the hero action row at 390; now scoped to >=834px plus [data-past-hero]; no aria-hidden (ADV-F-3 / WCAG 4.1.2). New TC-MV-CVTAP-01 red-then-green; TC-MV-CLICK-01 re-keyed on paint. tsc/lint/build/audit 10/10; 8/8 MiniVic contracts + occlusion + send-path green (W3-P0A logs 01-10). Fix 2 NOT delivered — diagnosed: F-2 as written does not reproduce on 199f116c (placement applies at all five viewports), the real defect is over-narrowing: 1440 panel 290 px, 1366 315 px (< TC-BOT-14 MIN 320, < 360 bar) because the panel is placed against the pre-font-settle name (right edge 1108 vs settled 1051.5) and no recompute path in placePanelClearOfHeroName can re-widen (early return on the cached natural box). F-4 reproduces at 390 only (composer outside a 368-px panel). Probe: W3-P0A/placement-probe.mjs (~40 s). Also found: TC-FOLD-04 @390 pre-existing FAIL on main (action group bottom 828 > 804) — the CTAs sit below the fold at 390 on live; covered by t_w3_rm1 TC-IF-02. -> t_w3_p0b.
