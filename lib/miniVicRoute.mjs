...
/**
 * The URL a FALLBACK send POSTs to — the rewrite plus the flag that tells the
 * function which of its two front doors this request came in on.
 *
 * The function shortens its answer on this route alone, because Firebase's edge
 * buffers the reply and its first byte is therefore the origin's completion time
 * (functions/index.js CHAT_MAX_TOKENS_FALLBACK, and the independent measurement
 * behind it in
 * docs/delivery/evidence/v10-20260905T0515Z/G-REV/97e19d07/08-adversarial-review.md
 * F-1). It can only do that if it knows the route, and the one signal no proxy
 * can garble is the client naming it. Header sniffing stays in the function as a
 * fallback for bundles cached from before this change; it is not the contract.
 *
 * The flag is on the SEND url only. The warm ping keeps its own query string —
 * it answers 204 and touches no provider, so a ceiling means nothing to it — and
 * the direct rung stays a bare origin POST so its 128-token ceiling, and its
 * measured 725-978 ms first token, are untouched.
 */
export const HOSTING_CHAT_SEND_URL = `${HOSTING_CHAT_ENDPOINT}?route=hosting`;
...
/**
 * Drop the incomplete tail a hard output ceiling leaves behind.
 *
 * The fallback route runs a 48-token ceiling (functions/index.js
 * `CHAT_MAX_TOKENS_FALLBACK`), and a ceiling does not negotiate: measured
 * against the deployed function on 2026-09-06, a capped reply ended
 * "…and delivering over 95% of the" (W1-M4B/04-hosting-verify.json). Shipping
 * that is a worse defect than the latency it bought — the sentence is not just
 * short, it is broken, and a reader cannot tell a truncation from a model that
 * lost its thread.
 *
 * So the visible answer stops at the last sentence the model actually finished.
 * Nothing is added and nothing is rephrased: this only removes a fragment that
 * was never a whole clause. When the ceiling landed before ANY sentence ended
 * there is nothing to fall back to, so the fragment is kept — a partial answer
 * is more use than an empty one — with an ellipsis so the cut is visible rather
 * than passed off as a finished thought.
 *
 * @param {string} text
 * @returns {string}
 */
export function trimCappedAnswer(text) {
  const whole = String(text ?? '').trim();
  if (!whole) return whole;
  let end = -1;
  // A terminator only ends a sentence when whitespace or the string end follows
  // it, so "95%." closes and "gpt-4.1-mini" does not.
  for (const match of whole.matchAll(/[.!?]["')\]]?(?=\s|$)/g)) {
    end = match.index + match[0].length;
  }
  if (end > 0) {
    const kept = whole.slice(0, end).trim();
    // A single surviving word plus a full stop is not an answer; a longer
    // fragment is more informative than that, so fall through to the clause cut.
    if (kept.length >= 40) return kept;
  }
  // No finished sentence. Fall back to the last CLAUSE the model closed — the
  // measured capped replies all ended mid-clause after several complete ones
  // ("…, executive reporting, and delivering over 95% of the"), and cutting at
  // that last comma leaves a readable, entirely model-written phrase. The
  // ellipsis is the honest part: it says the answer was cut, so nobody reads a
  // clause as a finished thought.
  const clause = whole.search(/[,;:](?=[^,;:]*$)/);
  if (clause >= 40) return `${whole.slice(0, clause)}…`;
