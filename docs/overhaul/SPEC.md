...
| DEV-5 | **Total Autonomy** — no confirmation dialogs; **publish directly to production** (prompt §7) | Build, test, and V&V run fully autonomously. The **final production publish remains owner-gated** per the owner's standing local-first workspace rule (`~/.codex/AGENTS.md`, `CLAUDE.md`), which overrides the direct-to-production clause. To honour full autonomy instead, flip MVP-AND-ROLLOUT §4 to auto-deploy on all-green CI. | **Deviation recorded — owner decision** |
