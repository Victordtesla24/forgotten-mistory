...
| 6 | `dab4b50` | The production credit restored — one line of authorship in the footer naming MiniVic's model-generated likeness and cloned-voice greeting; dead `my-avatar-voice.mp3` (501,178 B) removed | tsc · lint · audit 10/10 · build · content+listen+a11y 35/35 | Credit live and verbatim; `/assets/my-avatar-voice.mp3` → **404**; stamp `dab4b50` |
| 7 | `44603da` | R-180 adjudication: the terminal never existed, only its styles — `#terminal-*`, `.prompt`, dead `#mini-vic-*` id rules and five comment-only `@media` blocks removed via a postcss AST walk. globals.css 35,344 → 33,511 B | tsc · lint · audit 10/10 · build · content+a11y+monochrome+navigation 43/43 | `GET /` 200 |
| 8 | `f701558` | A dead global `footer{text-align:center}` rule, revived by adding a real footer, centred the page's last word — deleted; stray space before a comma fixed; README brought level (audit badge 9/9 → 10/10, avatar section replaced) | tsc · lint · audit 10/10 · build · content+a11y 27/27 · **screenshotted and inspected at 1440 and 390** | Footer left-aligned at 176px, exactly the hero's h1 |
| 9 | `c116106`+`a4800d2`+`1672d17` | T-37 implemented (210 preserved claims checked against production) and the build stamp taught to refuse a commit it was not built from — see `INCIDENT-01` | tsc · lint · audit 10/10 · build · guard **proved able to fail** | Deployed from a clean detached worktree; production stamp `1672d17` == `main` `1672d170`; leaked `var(--space-2)` count now **0** |
