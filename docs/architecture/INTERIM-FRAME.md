...
| `tests/e2e/vitrine.spec.ts` → `TC-VIT-05`'s six-drawing block (the "no screenshots, logos or raster images" assertion stays), and `TC-VIT-10`, `TC-VIT-11`, `TC-VIT-13` whole | Six inline `svg[role="img"]` mechanism drawings, each with a non-empty `title` and `desc`; a plate tracing its drawing as the light reaches it; the drawings present but untraced under reduced motion; every stroke of a lit plate reaching `stroke-dashoffset` 0 inside the cinematic band | TC-IF-14 (what each card must still carry, and the rail the keyboard can still drive) and TC-IF-19 (the same words and rows on both motion paths) |
