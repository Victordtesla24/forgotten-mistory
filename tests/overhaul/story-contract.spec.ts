...
for (const viewport of VIEWPORTS) {
  ...
  test.describe(`TC-STORY @ ${w} — every field says its own section`, () => {
    ...
    /* TC-STORY-HERO-01 and TC-STORY-ABOUT-01/02/03 are SUPERSEDED by
       tests/overhaul/interim-frame.spec.ts (TC-IF-01, TC-IF-03, TC-IF-05).
       They asserted that the hero plane's brightest region is where the figure
       is, and that the About fan reads as a ring of ten with three open
       sectors. The plane, the atmosphere, the compass and its field were
       removed by the Owner's 2026-09-06T05:51Z instruction
       (docs/architecture/INTERIM-FRAME.md); a case that measures a deleted
       element is not a case that can be made to pass. The four sections this
       slice did not touch keep every row below. */
